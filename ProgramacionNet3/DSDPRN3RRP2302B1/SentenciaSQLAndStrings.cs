...
namespace DSDPRN3RRP2302B1
{
    public class SentenciaSQLAndStrings
    {
        ...
        public const string SQL_OBTENER_PACIENTES_WITHOUT_DOCTOR_RRP = "SELECT tbpacientesrrp.idPacientes as Id, Nombre, ApellidoPaterno as 'Apellido Paterno', ApellidoMaterno as 'Apellido Materno', Direccion, Celular, TelefonoFijo as 'Telefono Fijo', Email, Edad, Sexo, Tipo as 'Estado civil' FROM tbpacientesrrp INNER JOIN tbestadocivilrrp ON tbpacientesrrp.idEstadoCivil = tbestadocivilrrp.idEstadoCivil LEFT JOIN tbpacientesmedicosrrp ON tbpacientesrrp.idPacientes = tbpacientesmedicosrrp.idPacientes WHERE tbpacientesmedicosrrp.idPacientesMedico IS NULL ORDER By tbpacientesrrp.idPacientes ASC";
        ...
        public const string SQL_OBTENER_PACIENTE_MEDICO_ESPECIALIDAD = "SELECT tbpacientesmedicosrrp.idPacientesMedico AS Id, CONCAT(tbpacientesrrp.Nombre,' ',tbpacientesrrp.ApellidoPaterno,' ', tbpacientesrrp.ApellidoMaterno) AS Paciente, CONCAT(tbmedicosrrp.Nombre,' ',tbmedicosrrp.ApellidoPaterno,' ',tbmedicosrrp.ApellidoMaterno) AS Médico,tbespecialidadesrrp.Nombre AS Especialidad FROM tbpacientesrrp INNER JOIN tbpacientesmedicosrrp  ON tbpacientesrrp.idPacientes = tbpacientesmedicosrrp.idPacientes INNER JOIN tbmedicosrrp ON tbpacientesmedicosrrp.idMedicos = tbmedicosrrp.idMedicos INNER JOIN tbespecialidadesrrp ON tbmedicosrrp.idEspecialidades = tbespecialidadesrrp.idEspecialidades ORDER By idPacientesMedico ASC;";
